...
plot_o2vt = o2_ht %>% 
  ggplot()+
  geom_line(aes(x=decy,y = `O2(1)`),color = 'red')+
  ylab("\nOxygen (µmol/kg)")+
  xlab("Time\n")+
  ggtitle("Oxygen Time Series at Hydrostation S (1955-Present)")+
  scale_x_continuous(limits = c(1955,2022), expand = c(0,0),breaks = seq(1950,2020,10))+
...
Oxygen ranges from a low of 155.8 µmol/kg to a maximum of 270.5 µmol/kg over the late 1960s to present day. We see higher oxygen values from the mid 1980s to the mid 1990s which can be an indicator of a positive NAO in contrast to the probable negative NAO trend that followed the mid 1990s into present. In the summary statistics we see a slight negative slope of oxygen concentrations (-0.07359) over time that is not statistically significant but is a possible indicator of anthropogenic climate change decreasing oxygen concentrations in the North Atlantic.
